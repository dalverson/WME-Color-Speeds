...
function colorSpeeds() {
    ...
    let loadStartTime;
    let CSpeedI18n;
    let CSCountry = sdk.DataModel.Countries.getTopCountry();
    let CSTopState = sdk.DataModel.States.getTopState();
...
*/
    ...
    function color2Rgb(c) {
        ...
        function parseRgb(arr) {
            ...
            if (debug)
                console.debug(`Red: ${rgb.r}, Green: ${rgb.g}, Blue: ${rgb.b}`);
            return rgb;
        }
    ...
    function showScriptInfoAlert() {
        ...
        GM_info.script.name, 
        // eslint-disable-next-line camelcase
        GM_info.script.version, changelogLocalizer(), greasyForkUrl, forumUrl);
    }
    ...
    function init() {
        ...
        if (localStorage.WMEColorSpeeds !== undefined && IsJsonString(localStorage?.getItem("WMEColorSpeeds"))) {
            ...
            if (WMECSpeeds.speedColors.Others === undefined)
                WMECSpeeds.speedColors.Others = "#f00";
            if (WMECSpeeds.speedColors.US === undefined || Object.keys(WMECSpeeds.speedColors.US).length === 0)
                WMECSpeeds.speedColors.US = structuredClone(colorsUS);
            if (WMECSpeeds.speedColors.Countries === undefined)
                WMECSpeeds.speedColors.Countries = {};
            if (WMECSpeeds.multiplePalette === undefined)
                WMECSpeeds.multiplePalette = multiplePalette;
            if (WMECSpeeds.PaletteByCountrie === undefined)
                WMECSpeeds.PaletteByCountrie = paletteByCountry;
            if (WMECSpeeds.offsetValue === undefined)
                WMECSpeeds.offsetValue = offsetValue;
            if (WMECSpeeds.opacityValue === undefined)
    ...
    function setupPanel() {
        ...
        if (WMECSpeeds.PaletteByCountrie && !WMECSpeeds.MultiplePalette) {
            if ($("#selectCountry").val() !== undefined) {
                ...
                selectedCountry = CSCountry.name.replace(/ /g, "_");
    ...
    function setupHandlers() {
        ...
        $("#cbPaletteByCountrie").off().on("click", function () {
            $("#selectCountry").css("display", this.checked ? "block" : "none");
            WMECSpeeds.PaletteByCountrie = this.checked;
        ...
        $("#cbMultiplePalette").off().on("click", function () {
            ...
            WMECSpeeds.MultiplePalette = this.checked;
        ...
        $("#CSColorSubmit").off().on("click", (e) => {
            if (debug)
                log("CSColorSubmit clicked", e);
            let newSpeed = $("#newspeed").val();
            let newColor = $("#ConfColor").css("background-color");
    ...
    function updateCountriesList() {
        ...
        let current = null;
        if (selectCountry.prop("selectedIndex") >= 0)
            current = selectCountry.val();
        if (current === null)
            current = CSCountry.name.replace(/ /g, "_");
        ...
        if (WMECSpeeds.speedColors?.Countries !== undefined && Object.keys(WMECSpeeds.speedColors.Countries).length > 0) {
            for (let country in WMECSpeeds.speedColors?.Countries) {
                ...
                if (country === undefined)
                    country = CSCountry.name.replace(/ /g, "_");
    ...
    function CSModifCouleur(id, color) {
        $("#Conf_Others").css("display", "none");
        $("#Conf_Color").css("display", "block");
        $("#colorspeedsDiv").css("display", "none");
        $("#newspeed").val(id);
    ...
    function SCSpeeds(unit, idx, state, contrie) {
        ...
        if (WMECSpeeds.MultiplePalette === true) {
            answer = unsafeWindow.confirm(`${CSlang[5][CSI18n]} ${idx} ${unit} ${CSlang[21][CSI18n]} ${state} ?`);
        }
        else if (WMECSpeeds.PaletteByCountrie === true) {
            answer = unsafeWindow.confirm(`${CSlang[5][CSI18n]} ${idx} ${unit} ${CSlang[21][CSI18n]} ${contrie} ?`);
        }
        else {
    ...
    function shiftGeometry(d, line, trigo) {
    ...
    function SCColor() {
        ...
        for (const seg of sdk.DataModel.Segments.getAll()) {
            ...
            let country;
            let state = segmentAddress.state?.name.replace(/ /g, "_");
            ...
            if (RoadToScan.indexOf(roadType) === -1)
                continue;
            if (roundabout !== false && WMECSpeeds.typeOfRoad["999"].checked === false)
                continue;
            if (unit === "mph") {
            ...
            if (fwdspeed && WMECSpeeds.visibility) {
                ...
                let newWidth, newColor, newDashes = "", newOpacity;
                if (!WMECSpeeds.MultiplePalette && !WMECSpeeds.PaletteByCountrie) {
        ...
        if (debug)
            log(`Total load time without WW of ${Math.round(performance.now() - scriptStartTime)} ms.`);
        if (debug)
            log(`Total load time with WW of ${Math.round(performance.now() - loadStartTime)} ms.`);
    }
